...
###### Ответ:
Что-бы получить `stderr` в качестве входного потока для команды стоящей справа от `|`:

```bash
(dir && tt) 3>&1 1>&2 2>&3 | wc
3  fff  sss  wc
      1       5      29
```
...
Без перенаправления:
```bash
vagrant@vagrant:/tmp/test$ dir
3  fff  sss  wc
vagrant@vagrant:/tmp/test$ (dir && tt) | wc
